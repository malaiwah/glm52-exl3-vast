...
def layer_norm_gated_fwd_kernel(
    ...
    if HAS_RESIDUAL:
        b_x += tl.load(residual + offsets, mask=mask, other=0.0).to(tl.float32)
    if STORE_RESIDUAL_OUT:
        tl.store(
            residual_out + offsets,
            b_x.to(residual_out.dtype.element_ty),
            mask=mask,
        )
    if not IS_RMS_NORM:
...
def chunk_kda_scaled_dot_kkt_fwd_kernel_intra_sub_inter(
    ...
    o_i = tl.arange(0, BC)
    rows_i = i_t * BT + i_i * BC + o_i
    rows_i_mask = rows_i < T
    b_b = tl.load(
        beta + (bos + rows_i) * H + i_h,
        mask=rows_i_mask,
        other=0.0,
    )
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        k_mask = o_k < K
        rows_j = i_t * BT + i_j * BC + tl.arange(0, BC)
        rows_j_mask = rows_j < T
        offsets_i = rows_i[:, None] * H * K + o_k[None, :]
        mask_i = rows_i_mask[:, None] & k_mask[None, :]
        offsets_j = o_k[:, None] + rows_j[None, :] * H * K
        mask_j = k_mask[:, None] & rows_j_mask[None, :]
        ...
        b_g = tl.load(g + offsets_i, mask=mask_i, other=0.0)
        b_k = tl.load(k + offsets_i, mask=mask_i, other=0.0) * exp2(
            b_g - b_gn[None, :]
        )
        b_gk = tl.load(g + offsets_j, mask=mask_j, other=0.0)
        b_kt = tl.load(k + offsets_j, mask=mask_j, other=0.0)
        # [BC, BC]
        b_ktg = b_kt * exp2(b_gn[:, None] - b_gk)
        b_A += tl.dot(b_k, b_ktg)

        b_q = tl.load(q + offsets_i, mask=mask_i, other=0.0)
        b_qg = b_q * exp2(b_g - b_gn[None, :]) * scale
        b_Aqk += tl.dot(b_qg, b_ktg)

    b_A *= b_b[:, None]

    columns_j = i_j * BC + tl.arange(0, BC)
    output_offsets = rows_i[:, None] * H * BT + columns_j[None, :]
    output_mask = rows_i_mask[:, None] & (columns_j[None, :] < BT)
    tl.store(
        A + output_offsets,
...
def chunk_kda_scaled_dot_kkt_fwd_kernel_intra_sub_intra(
    ...
    o_k = tl.arange(0, BK)
    m_k = o_k < K
    rows = i_t * BT + i_i * BC + o_i
    m_A = rows < T
    o_A = (bos + rows) * H * BT + i_h * BT + i_i * BC

    offsets = ((bos + rows[:, None]) * H + i_h) * K + o_k[None, :]
    mask = m_A[:, None] & m_k[None, :]
    b_q = tl.load(q + offsets, mask=mask, other=0.0)
    b_k = tl.load(k + offsets, mask=mask, other=0.0)
...
def recompute_w_u_fwd_kernel(
    ...
    o_a = tl.arange(0, BT)
    offsets_a = ((bos + o_t[:, None]) * H + i_h) * BT + o_a[None, :]
    mask_a = m_t[:, None]
    b_A = tl.load(A + offsets_a, mask=mask_a, other=0.0)

    for i_v in range(tl.cdiv(V, BV)):
        o_v = i_v * BV + tl.arange(0, BV)
        offsets_v = ((bos + o_t[:, None]) * H + i_h) * V + o_v[None, :]
        mask_v = m_t[:, None] & (o_v[None, :] < V)
        b_v = tl.load(v + offsets_v, mask=mask_v, other=0.0)
    ...
    for i_k in range(tl.cdiv(K, BK)):
        o_k = i_k * BK + tl.arange(0, BK)
        offsets_k = ((bos + o_t[:, None]) * H + i_h) * K + o_k[None, :]
        mask_k = m_t[:, None] & (o_k[None, :] < K)
        b_k = tl.load(k + offsets_k, mask=mask_k, other=0.0)
        b_kb = b_k * b_b[:, None]
        b_gk = tl.load(gk + offsets_k, mask=mask_k, other=0.0)
        b_kb *= exp2(b_gk)
        if STORE_QG:
            b_q = tl.load(q + offsets_k, mask=mask_k, other=0.0)
            b_qg = b_q * exp2(b_gk)
            tl.store(
                qg + offsets_k,
                b_qg.to(qg.dtype.element_ty),
                mask=mask_k,
            )
        if STORE_KG:
...
def chunk_gla_fwd_kernel_o(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        o_t = i_t * BT + tl.arange(0, BT)
        o_k = i_k * BK + tl.arange(0, BK)
        qg_offsets = ((bos + o_t[:, None]) * H + i_h) * K + o_k[None, :]
        qg_mask = (o_t[:, None] < T) & (o_k[None, :] < K)
        o_v = i_v * BV + tl.arange(0, BV)
        h_offsets = (
            (i_tg * H + i_h) * K * V
            + o_v[:, None] * K
        ...
        h_mask = (o_v[:, None] < V) & (o_k[None, :] < K)

        # [BT, BK]
        b_q = tl.load(q + qg_offsets, mask=qg_mask, other=0.0)
        b_q = (b_q * scale).to(b_q.dtype)
        # [BT, BK]
        b_g = tl.load(g + qg_offsets, mask=qg_mask, other=0.0)
        # [BT, BK]
    ...
    o_t = i_t * BT + tl.arange(0, BT)
    o_v = i_v * BV + tl.arange(0, BV)
    value_offsets = ((bos + o_t[:, None]) * H + i_h) * V + o_v[None, :]
    value_mask = (o_t[:, None] < T) & (o_v[None, :] < V)
    o_a = tl.arange(0, BT)
    a_offsets = ((bos + o_t[:, None]) * H + i_h) * BT + o_a[None, :]
    a_mask = o_t[:, None] < T
    # [BT, BV]
    b_v = tl.load(v + value_offsets, mask=value_mask, other=0.0)
    # [BT, BT]
    b_A = tl.load(A + a_offsets, mask=a_mask, other=0.0)
...
def kda_gate_cumsum_fwd_kernel(
    ...
    offsets = ((bos + o_t[:, None]) * H + i_h) * D + o_d[None, :]
    ...
    # out-of-bounds output rows, which the store mask drops, so visible output
    # matches unfused gate + chunk-local cumsum.
    chunk_rows = tl.arange(0, BT)
    m_cumsum = tl.where(
        chunk_rows[:, None] >= chunk_rows[None, :],
        1.0,
        0.0,
    )
