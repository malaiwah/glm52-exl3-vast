...
class ApplyRotaryEmb(CustomOp):
    ...
    def forward_cuda(
        ...
        if (
            self.apply_rotary_emb_vllm_flash_attn is None
            and self.apply_rotary_emb_flash_attn is None
        ):
            logger.warning_once(
                "No FlashAttention rotary kernel is available; using the native "
                "PyTorch rotary implementation."
            )
            return self.forward_native(x, cos, sin)

        x, cos, sin, origin_shape, origin_dtype = self._pre_process(x, cos, sin)
        interleaved = not self.is_neox_style
        if self.apply_rotary_emb_vllm_flash_attn is not None:
            output = self.apply_rotary_emb_vllm_flash_attn(
                x, cos, sin, interleaved
            )
        else:
            assert self.apply_rotary_emb_flash_attn is not None
