...
class Exl3Config(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> Exl3Config:
        ...
        if config.get("scope") == _GLM53_ROUTED_EXPERTS_SCOPE:
            ...
            mismatches = {
                ...
                if config.get(key) not in (value if isinstance(value, tuple) else (value,))
            }
            if mismatches:
                raise ValueError(
    ...
    def _configure_glm53_unsliced_routed_experts(
        ...
        self.rank_sliced_metadata = {
            "bits": hf_config.quantization_config.get("bits", 4),
            "codebook": "mcg",
        ...
        logger.info_once(
            "GLM-5.3 routed-only EXL3: streaming unsliced EXL3 experts into "
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if rank_sliced_metadata is not None:
            ...
            # unsliced_tp_stream checkpoints store experts without a fixed TP
            # slice, so any runtime TP size is valid — the loader re-slices.
            unsliced = (
                rank_sliced_metadata.get("source_layout") == "unsliced_tp_stream"
            )
            if not ep_from_unsliced and not unsliced and checkpoint_tp != layer.exl3_tp_size:
                raise ValueError(
...
def warmup_exl3_mixed_trellis_route_pack(model: torch.nn.Module) -> int:
    """Materialize Trellis route-pack kernels before serving.

    The first profile pass creates every immutable Trellis runtime, but its
    route-pack planning does not reliably retain the int64 specialization used
    by vLLM's router output. Compile that specialization explicitly for each
    uniform prefill geometry. Mixed-bitrate runtimes still delegate their
    decode and prefill enumeration to B12X.
    """
    ...
    seen_uniform: set[tuple[object, ...]] = set()
    for runtime in _RANK_SLICED_RUNTIMES.values():
        if runtime["prefill_plan"] is None:
            continue
        device = runtime["xh"].device
        capacity = int(runtime["prefill_capacity"])
        topk = int(runtime["topk"])
        block_m = int(runtime["prefill_block_m"])
        num_experts = int(runtime["num_experts"])
        expert_map = runtime["route_expert_map"]
        signature = (
            device,
            capacity,
            topk,
        ...
        if signature in seen_uniform:
            continue
        seen_uniform.add(signature)
        for token_count in sorted({capacity, max(capacity - 1, 1)}):
