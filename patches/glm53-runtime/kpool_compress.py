...
INDEX_HEAD_DIM = 128
KPOOL_INDEX_HEADS = 32
KPOOL_PAGE_SIZE = 32


@triton.jit
def _kpool_paged_mqa_logits_32_kernel(
    q_ptr,
    kv_u8_ptr,
    kv_f32_ptr,
    weights_ptr,
    ...
    """Score one logical 32-row KPool page for one decode query.

    KPool's uint8 allocation is planar within each physical page: all FP8
    key rows first, then one FP32 scale per row.  This differs from the
    tensor's nominal ``[..., head_dim + 4]`` row view and is the reason the
    generic interleaved-cache scorer cannot consume it directly.
    """
    query_row = tl.program_id(0)
    logical_page = tl.program_id(1)

    slot = tl.arange(0, PAGE_SIZE)
    dim = tl.arange(0, HEAD_DIM)
    head = tl.arange(0, NUM_HEADS)
    token_pos = logical_page * PAGE_SIZE + slot
    seq_len = tl.load(seq_lens_ptr + query_row)

    physical_page = tl.load(
        block_table_ptr + query_row * block_table_stride_row + logical_page
    ).to(tl.int64)
    valid_page = (physical_page >= 0) & (physical_page < num_cache_pages)
    safe_page = tl.maximum(physical_page, 0)

    k_u8 = tl.load(
        kv_u8_ptr
        + safe_page * page_stride_bytes
        + slot[:, None] * HEAD_DIM
        + dim[None, :],
        mask=valid_page,
        other=0,
    ...
    q_fp8 = tl.load(
        q_ptr
        + query_row * q_stride_row
        + head[:, None] * q_stride_head
        + dim[None, :] * q_stride_dim
    )
    score = tl.dot(k_fp8, tl.trans(q_fp8), out_dtype=tl.float32)
    gate = tl.load(
        weights_ptr + query_row * weights_stride_row + head * weights_stride_head
    ).to(tl.float32)
    score = tl.maximum(score, 0.0) * gate[None, :]
    score = tl.sum(score, axis=1)

    scale = tl.load(
        kv_f32_ptr + (safe_page * page_stride_bytes + SCALE_OFFSET_BYTES) // 4 + slot,
        mask=valid_page,
        other=0.0,
    )
    valid_token = valid_page & (token_pos < seq_len)
    score = tl.where(valid_token, score * scale, -float("inf"))
    tl.store(
        logits_ptr + query_row * logits_stride_row + token_pos,
        score,
    )


def kpool_paged_mqa_logits_32(
    q_fp8: torch.Tensor,
    ...
    """Native paged FP8 MQA logits for GLM's 32-row KPool cache.

    The kernel writes only page-table-covered columns; the allocation starts
    at ``-inf`` so any graph-capacity tail stays invisible to top-k.
    """
    assert q_fp8.ndim == 3 and q_fp8.shape[1:] == (
        KPOOL_INDEX_HEADS,
        INDEX_HEAD_DIM,
    )
    assert q_fp8.dtype == torch.float8_e4m3fn
    assert kv_cache.ndim == 3 and kv_cache.dtype == torch.uint8
    assert kv_cache.shape[1:] == (KPOOL_PAGE_SIZE, INDEX_HEAD_DIM + 4)
    assert weights.shape == q_fp8.shape[:2]
    assert seq_lens.ndim == 1 and seq_lens.shape[0] == q_fp8.shape[0]
    assert block_table.ndim == 2 and block_table.shape[0] == q_fp8.shape[0]
    assert block_table.stride(1) == 1
    # The scheduler sizes block_table to physical KV capacity, which can be
    # much wider than the configured logical model length. Never score or
    # materialize those capacity-only columns.
    logical_page_count = min(
        block_table.shape[1], triton.cdiv(max_pool_len, KPOOL_PAGE_SIZE)
    )

    logits = torch.full(
        (q_fp8.shape[0], max_pool_len),
        -float("inf"),
        dtype=torch.float32,
        device=q_fp8.device,
    )
    if q_fp8.shape[0] == 0 or logical_page_count == 0:
        return logits
    _kpool_paged_mqa_logits_32_kernel[(q_fp8.shape[0], logical_page_count)](
        q_fp8,
        kv_cache,
        ...
        kv_cache.stride(0),
        kv_cache.shape[0],
        PAGE_SIZE=KPOOL_PAGE_SIZE,
        NUM_HEADS=KPOOL_INDEX_HEADS,
        HEAD_DIM=INDEX_HEAD_DIM,
        SCALE_OFFSET_BYTES=KPOOL_PAGE_SIZE * INDEX_HEAD_DIM,
        num_warps=4,
        num_stages=2,
    )
    return logits
...
def expand_pools_and_append_tail(
    ...
    if out is None:
        out = torch.empty(
            (rows, out_cols), dtype=torch.int32, device=pool_ids.device
        )
    else:
        if out.device != pool_ids.device:
            raise ValueError("out and pool_ids must be on the same device")
        if out.dtype != torch.int32:
            raise ValueError(f"out must have dtype torch.int32, got {out.dtype}")
        if out.ndim != 2 or out.shape[0] < rows or out.shape[1] < out_cols:
            raise ValueError(
                "out must cover the expanded result: "
                f"need ({rows}, {out_cols}), got {tuple(out.shape)}"
            )
        if out.stride(1) != 1:
            raise ValueError("out must be contiguous in its last dimension")
        out = out[:rows, :out_cols]
    BLOCK_COLS = 128
