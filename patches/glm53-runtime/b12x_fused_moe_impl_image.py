...
def _plan_full_rotation_w4a16_launches(
    *,
    ...
    with torch.cuda.device(core_plan.device):
        ...
        route_pack_token_counts = tuple(
            1 << shift for shift in range(capacity_tokens.bit_length())
        )
        if route_pack_token_counts[-1] != capacity_tokens:
            route_pack_token_counts += (capacity_tokens,)
        ...
        for route_ids_dtype in (torch.int32, torch.int64):
            for route_tokens in route_pack_token_counts:
                for mapped in (False, True):
                    route_pack_key = (
                        core_plan.device.type,
                        int(torch.cuda.current_device()),
                        route_ids_dtype,
                        route_tokens * core_plan.num_topk,
                        int(block_size_m),
