...
class B12xMLASparseImpl(MLAAttentionImpl[B12xMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        # Full-attention and backbone-skip layers receive the shared top-k
        # buffer but deliberately have no indexer. There is no producer output
        # format to validate on those layers; inferring one from index_kpool
        # rejects every non-IndexPool model at DCP > 1 before weight loading.
        if indexer is not None:
            indexer_outputs_physical = bool(indexer.output_physical_slots)
