...
class MLADCPManager:
        ...
        *,
        ...
        if direct_workspace is not None:
            ...
            return functools.partial(
                self._direct_combine_with_capacity_fallback,
    ...
    def _direct_combine_with_capacity_fallback(
        self,
        partial_output: torch.Tensor,
        partial_lse: torch.Tensor,
        *,
        ...
        fallback_combine: DCPCombine,
        is_lse_base_on_e: bool,
    ) -> torch.Tensor:
        """Use direct A2A for supported decode layouts and NCCL otherwise.

        ``get_dcp_workspace_max_num_tokens`` intentionally sizes direct
        symmetric memory for decode/spec-decode rows, which is far smaller than
        ``max_num_batched_tokens``. Sparse MLA also invokes the same combine for
        chunked prefill. Attention backends may also return head-major views
        that the direct kernel cannot consume. Route either case through the
        ordinary A2A collective.
        """
        fits_workspace = 0 < partial_output.shape[0] <= self.max_num_tokens
    ...
    def _init_query_gather(
        ...
        *,
        ...
        if direct_workspace is not None:
            ...
            return functools.partial(
                self._direct_query_gather_with_capacity_fallback,
                direct_workspace=direct_workspace,
            )
        return self._gather_query

    def _direct_query_gather_with_capacity_fallback(
        self,
        query: torch.Tensor,
        *,
        direct_workspace: DirectDCPQGatherWorkspace,
    ) -> torch.Tensor:
        if query.shape[0] <= self.max_num_tokens:
