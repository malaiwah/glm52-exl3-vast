...
def _uses_b12x_streaming_prefill(
    *, use_b12x_sparse_indexer: bool, index_kpool: int
) -> bool:
    """True only when the executing prefill op has a B12X paged route.

    GLM KPool decode uses B12X, but its prefill op still gathers compressed K
    rows and materializes dense DeepGEMM logits.  Treating it as streamed here
    underestimates that allocation by the ratio between the live pool context
    and B12X's fixed supertile budget.
    """
    return bool(use_b12x_sparse_indexer and index_kpool <= 1)
...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        # cache access (write via slot_mapping AND the top-k reads), so the pool
        # keys written at indexer-page granularity are read back from the same
        # physical pages.
        indexer_block_table = block_table
        if self.compress_ratio > 1:
            # The shared block_table is in kernel_block_size (token) units
            # because the co-located MLA is virtually split. The indexer is NOT
            # split and is addressed at storage_block_size pool-page
            # granularity, so translate: every (block_size // kernel_block_size)
            # columns is one indexer page, and each entry is
            # (page_phys * that factor), so dividing recovers the indexer
            # physical page id in [0, num_blocks). When the builder carries no
            # kernel_block_size the MLA is not split and the raw table is
            # already correct.
        ...
        if num_prefills > 0:
            ...
            if _uses_b12x_streaming_prefill(
                use_b12x_sparse_indexer=self.use_b12x_sparse_indexer,
        ...
        if num_decodes > 0:
            ...
            # B12X's native paged index scorer currently consumes 64 index
            # rows per page.  GLM KPool4 can have a 32-row physical index page
            # after hybrid-cache alignment; in that case retain B12X for MLA
            # but build DeepGEMM scheduling metadata for the index scorer,
            # which natively supports both 32- and 64-row pages.
            use_b12x_paged_scorer = (
                self.use_b12x_sparse_indexer
                and self.kv_cache_spec.storage_block_size == 64
            )
            active_width: torch.Tensor | None = None
            decode_topk_max_seq_len: int | None = None
            if use_b12x_paged_scorer:
