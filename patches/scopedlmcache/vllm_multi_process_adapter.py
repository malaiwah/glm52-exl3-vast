...
class ExtraConfigDefault(enum.Enum):
    ...
    # Submission through device-event readiness; zero disables the deadline.
    # Expiry is fatal: there is no cancellation ACK protecting target KV blocks.
...
class LMCacheMPWorkerAdapter:
    def __init__(
        ...
        *,
        ...
        self._retrieve_fatal_error: LMCacheTimeoutError | None = None
        ...
        # Block IDs of completed retrieves with a failed result, consumed by
        # get_block_ids_with_load_errors(). Never unacknowledged DMA.
    ...
    def submit_retrieve_request(
        ...
        """
        self._ensure_heartbeat_started()
        if self._retrieve_fatal_error is not None:
            raise self._retrieve_fatal_error
        ...
        self._retrieve_started_at[request_id] = started_at
    ...
    def _fail_stop_retrieve(self, request_id: str, reason: str) -> NoReturn:
        # Keep futures, exporting events and block ownership intact. Even a
        # late completion cannot make an expired transfer recoverable.
        if self._retrieve_fatal_error is None:
            self._retrieve_fatal_error = LMCacheTimeoutError(
                f"LMCache retrieve for request_id={request_id}: {reason}; "
                "aborting the worker: in-flight DMA has no cancellation ACK "
                "and target KV blocks cannot safely be reused",
    ...
    def get_finished(
        ...
        Returns:
            ...
            - The second set contains the finished retrieve request ids,
                ...
                A completed retrieve with result False or a raising result()
                is reported finished with its block ids recorded in
                error_block_ids so the scheduler recomputes them.

        Raises:
            LMCacheTimeoutError: A deadline expired, completion query failed,
                or health was lost with submitted retrieves. This is fatal,
                not a cache miss, because DMA may still be in flight.
        ...
        if self._retrieve_fatal_error is not None:
            raise self._retrieve_fatal_error
        if not self.is_healthy:
            # Heartbeat failure is not a transfer-cancellation ACK. Never
            # allow degraded-mode draining to bypass the ownership fence.
            if self.retrieve_futures:
                self._fail_stop_retrieve(
                    next(iter(self.retrieve_futures)),
                    "server health lost during retrieve",
                )
            ...
            # A dropped (never submitted) retrieve may also be engine-finished.
            # Do not report both receiving and sending completion for it.
        ...
        for request_id, (r_future, r_block_ids) in self.retrieve_futures.items():
            try:
                ready = r_future.query()
            except Exception as exc:
                # query() imports and polls the CUDA IPC event. An exception
                # here is not proof that DMA has finished, even if it carries
                # a remote error. Never make these target blocks reusable.
                self._fail_stop_retrieve(
                    request_id, f"completion query failed before DMA acknowledgement: {exc!r}"
                )
            if not ready:
                if (
                    self._retrieve_timeout > 0
                    and time.monotonic() - self._retrieve_started_at[request_id]
                    >= self._retrieve_timeout
                ):
                    self._fail_stop_retrieve(
                        request_id,
                        f"device completion exceeded {self._retrieve_timeout:.3f}s",
                    )
                continue
            try:
                # Only a successful completion query establishes that result
                # failures can follow the ordinary invalidation/recompute path.
                r_result = r_future.result()
            except TimeoutError:
                self._fail_stop_retrieve(request_id, "retrieve future timed out")
            except Exception as exc:
                logger.error(
                    "LMCache completed retrieve result raised for request_id=%s: %r "
    ...
    def get_block_ids_with_load_errors(self) -> set[int]:
        """
        Returns the block IDs of terminal failed retrieves (server-reported
        failure or raising result after completion), then clears the internal set.
        """
