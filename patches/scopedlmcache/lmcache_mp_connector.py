...
def scopedlmcache_model_name(vllm_config: "VllmConfig") -> str:
    """Isolate opaque per-rank KV pages by their canonical shared layout.

    This exact vLLM build's validate_block_size gives the legacy DCP option
    precedence when it is greater than one. Resolve that same value without
    mutating vLLM state, so scheduler and worker agree before/after validation.
    No worker-only tensor shape, rank, or implicit interleave fallback is used.
    A versioned namespace intentionally leaves pre-fix disk objects cold.
    """
    ...
    if dcp > 1 and pc.dcp_kv_cache_interleave_size > 1:
        interleave = pc.dcp_kv_cache_interleave_size
    if dcp < 1 or block < 1 or interleave < 1:
        raise ValueError("LMCache requires positive DCP, block and interleave sizes")
    if dcp > 1 and (interleave > block or block % interleave):
        raise ValueError("LMCache DCP interleave must divide the physical block size")
    return (
        f"{vllm_config.model_config.model}##appliance-kv-layout-v1"
        f"-d{dcp}-b{block}-i{interleave}"
    )
...
class LMCacheMPConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        cache_model_name = scopedlmcache_model_name(vllm_config)
