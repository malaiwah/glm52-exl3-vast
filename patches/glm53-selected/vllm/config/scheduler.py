...
class SchedulerConfig:
    ...
    fairness_engine: Literal["compute_share"] | None = None
    """Opt-in measured model-service wallclock-share scheduling. Unset preserves
    the legacy scheduler, including prefill_schedule_interval."""

    prefill_compute_share: float | None = Field(default=None, gt=0.0, lt=1.0)
    """Target prefill share of contended model-service wallclock, not GPU-only
    time. Requires fairness_engine=compute_share and leaves both shares positive."""
    ...
    def __post_init__(self, max_model_len: int, is_encoder_decoder: bool) -> None:
        if self.fairness_engine is None and self.prefill_compute_share is not None:
            raise ValueError("prefill_compute_share requires fairness_engine=compute_share")
        if self.fairness_engine == "compute_share":
            if self.prefill_compute_share is None:
                raise ValueError("compute_share requires prefill_compute_share")
            if self.prefill_schedule_interval != 1:
                raise ValueError("compute_share requires prefill_schedule_interval=1")
            if not self.enable_chunked_prefill or is_encoder_decoder:
                raise ValueError("compute_share requires decoder-only chunked prefill")
            if self.scheduler_cls is not None:
                raise ValueError("compute_share requires the built-in scheduler")
