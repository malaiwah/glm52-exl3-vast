...
read_ttl_seconds="${LMCACHE_L1_READ_TTL:-300}"
session_ttl_seconds="${LMCACHE_SESSION_TTL_SECONDS:-600}"
...
args=(server --host "$host" --port "$port" --chunk-size "$chunk"
  ...
  --l1-read-ttl-seconds "$read_ttl_seconds"
...
echo ">>> LMCache ready: mode=$mode L1=${l1_gb}GiB (initial ${l1_init_gb}GiB, lazy growth) chunk=$chunk GPU-workers=$gpu_workers read-TTL=${read_ttl_seconds}s session-TTL=${session_ttl_seconds}s L2=$l2 metrics=http://127.0.0.1:${http_port}/metrics"
