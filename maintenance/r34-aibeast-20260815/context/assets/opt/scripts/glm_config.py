...
# A later prompt_logprobs workload proved that a block-count override bypasses
# vLLM's profiled safety budget: the engine had 218.81 MiB free when a 304 MiB
# TP logits all-gather arrived. r31 prewarms the late mixed-Trellis route
# kernels; the #258 overlay profiles and bounds prompt-logprobs. The final r31
# gate instead pins the *byte* pool to exactly 520,192 logical dynamic-NVFP4
# tokens. This keeps the full request envelope while preserving roughly
# 0.84--1.01 GiB/rank after concurrent prompt-logprobs and 128K cold prefill.
VARIANTS["exl3-tr3-3.42bpw"] = {
    "family": "glm52",
    "label": "EXL3-TR3 shared-H 3.42bpw + online K6 — safe high-fidelity 520K",
...
VARIANTS["exl3-tr3-3.42bpw"]["defaults"].update({
    ...
    "GPU_BLOCKS_OVERRIDE": 0,
    # 520,192 logical tokens * 8,687 bytes/token/rank. Unlike the testing-only
    # block override, the explicit byte pool leaves vLLM's non-KV memory
    # accounting intact and makes the intended safety reservation auditable.
    "KV_CACHE_MEMORY_BYTES": 4518907904,
    # C12 is the measured concurrency knee. It passed 96 C1/C4/C8/C12 requests,
    # four concurrent 16K prompt-logprobs requests, and 64K/128K cold prefill.
    # C8/4096 OOMed on the first 48 MiB runtime activation, so PP stays 3,072.
    "MAX_NUM_SEQS": 12,
    "MAX_NUM_BATCHED_TOKENS": 3072,
    "VLLM_EXL3_PREFILL_CAPACITY": 3072,
