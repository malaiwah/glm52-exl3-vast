...
class Exl3MoEParameter(BasevLLMParameter):
    ...
    def __new__(
        ...
        *,
        ...
        del num_experts, shard_ids, preallocate, preallocate_groups, tp_slice
        data = torch.empty(0, dtype=torch.uint8)
        return super().__new__(cls, data=data, weight_loader=weight_loader)
    ...
    def __init__(
        ...
        *,
        ...
        if self.exl3_preallocate_groups and self.exl3_preallocate:
            raise ValueError(
                "EXL3 grouped and whole-slab preallocation are mutually exclusive"
            )
        if self.exl3_preallocate_groups and tp_slice is not None:
            raise ValueError(
                "EXL3 grouped preallocation cannot use the R7 streaming TP slice"
            )
    ...
    def load_exl3_weight(
        ...
        *,
        ...
        if self.exl3_preallocate_groups:
            ...
            # This synchronous copy is the lifetime boundary for an optional
            # borrowed InstantTensor staging buffer. The tier slab owns the
            # data before the loader is allowed to reuse that buffer.
            target.copy_(loaded_weight, non_blocking=not borrowed)
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def _prepare_mixed_rank_sliced_weights(self, layer: RoutedExperts) -> None:
        ...
        for bits, expert_ids in tiers.items():
            ...
            if isinstance(w13_param, Exl3MoEParameter) and isinstance(
