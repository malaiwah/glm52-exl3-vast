    ...
    def __post_init__(self):
        ...
        if self.scheduler_config.fairness_engine == "compute_share":
            if (
                ...
                raise ValueError(
                    "compute_share requires DP=PP=PCP=1 and no DBO; "
                    "V1/V2 model runners, TP and decode context parallelism are supported"
                )
            if (
                self.speculative_config is not None
                and self.speculative_config.method != "mtp"
            ):
                raise ValueError("this compute_share port supports MTP or no speculation")
