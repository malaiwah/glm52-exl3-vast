...
class PrometheusStatLogger(AggregateStatLoggerBase):
    ...
    def __init__(
        ...
        gauge_compute_share = self._gauge_cls(
            name="vllm:prefill_compute_share",
            documentation="Target prefill share of contended model-service wallclock; zero disables.",
            multiprocess_mode="mostrecent",
            labelnames=labelnames,
        )
        self.gauge_compute_share = create_metric_per_engine(
            gauge_compute_share, per_engine_labelvalues
        )
        counter_compute_seconds = self._counter_cls(
            name="vllm:scheduler_compute_seconds",
            documentation="Observed contended model-service wallclock by class, not GPU-only time.",
            labelnames=labelnames + ["class"],
        )
        self.counter_compute_seconds = {
            service_class: create_metric_per_engine(
                counter_compute_seconds,
                {
                    idx: values + [service_class]
    ...
    def record(
        ...
        """Log to prometheus."""
        if scheduler_stats is not None:
            self.gauge_compute_share[engine_idx].set(
                scheduler_stats.prefill_compute_share or 0.0
            )
            self.counter_compute_seconds["decode"][engine_idx].inc(
                scheduler_stats.decode_compute_seconds
            )
            self.counter_compute_seconds["prefill"][engine_idx].inc(
