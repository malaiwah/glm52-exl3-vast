...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        share = self.scheduler_config.prefill_compute_share
        self.compute_share_controller = (
            PrefillComputeShareController(share)
            if self.scheduler_config.fairness_engine == "compute_share"
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        scheduled_prefill_req_ids: set[str] | None = (
            set() if self.compute_share_controller is not None else None
        )
        ...
        adaptive_defer_prefills = compute_contention and selected_compute_class == "decode"
        adaptive_prefill_turn = compute_contention and selected_compute_class == "prefill"
        ...
        def schedule_running_requests(
            ...
            *,
            ...
            nonlocal token_budget, encoder_compute_budget, prefill_scheduled
            req_index = 0
            while req_index < len(self.running) and token_budget > 0:
                request = self.running[req_index]
                if service_class is not None and (
                    ("prefill" if request.is_prefill_chunk else "decode") != service_class
                ):
                    req_index += 1
                    continue
                if running_req_ids_at_step_start is not None and (
                ...
                if scheduled_prefill_req_ids is not None and request.is_prefill_chunk:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if load_kv_async:
                    ...
                    has_local_prefill = num_computed_tokens < request.num_tokens - 1
                    if defer_prefills and has_local_prefill:
                        if adaptive_defer_prefills and not num_scheduled_tokens:
                            # Async guards or resource checks made decode unrunnable.
                ...
                num_scheduled_tokens[request_id] = num_new_tokens
                if (
                    scheduled_prefill_req_ids is not None
        ...
        if (
            ...
            # Async guards or resource checks can disprove coarse eligibility.
            defer_prefills = False
            schedule_running_requests("prefill")

        if adaptive_prefill_turn and token_budget > 0:
            # Upstream semantics: prefills get first use of the quantum;
            # decodes consume the remainder without preempting admitted work.
            # The whole mixed batch is charged to the prefill service class.
            schedule_running_requests(
                "decode", allow_preemption=False, enforce_lora_limit=True
            )
        ...
        scheduler_output = SchedulerOutput(
            ...
            compute_service_class=(
                ...
                if scheduled_prefill_req_ids is not None
                and compute_contention and total_num_scheduled_tokens > 0
                else None
            ),
    ...
    def record_compute_time(
        self, service_class: ComputeServiceClass, elapsed_seconds: float, *, contended: bool
    ) -> None:
        """Charge measured model-service wallclock from the completed batch."""
        if self.compute_share_controller is None or not contended:
            return
        self.compute_share_controller.record(service_class, elapsed_seconds, contended=True)
        if self.log_stats:
