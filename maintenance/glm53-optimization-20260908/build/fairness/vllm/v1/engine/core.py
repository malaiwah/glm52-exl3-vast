...
class _ModelExecutionTiming:
    """Measure model-service wallclock, not GPU-only execution time.

    Includes IPC and output handling. Multiproc future completion is observed
    when the response is consumed, so this is not an accelerator timestamp.
    """
...
class EngineCore:
    ...
    def _record_compute_time(
        ...
        # Upstream accounting subtracts overlapping queue residence. This is
        # an observed model-service interval, not isolated accelerator compute.
