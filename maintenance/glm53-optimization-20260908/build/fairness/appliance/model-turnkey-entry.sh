...
# Experimental startup-only selector; unset preserves the baseline command.
if [ -n "${VLLM_PREFILL_FAIRNESS_ENGINE:-}" ]; then
  _SERVE_CMD+=(--fairness-engine "$VLLM_PREFILL_FAIRNESS_ENGINE"
    --prefill-compute-share "${VLLM_PREFILL_COMPUTE_SHARE:?set the measured-service share}")
fi
