...
| `PREFIX_CACHE_BACKEND` | `native` is the measured in-process control. `lmcache` uses r11's supervised DCP-aware LMCache MP connector with independent health and metrics. The same DRAM fraction applies; changing backend requires an evict/reload correctness and latency gate. |
| `PREFIX_CACHE_DISK_GB` | 0 is DRAM-only. A positive value enables LMCache's native filesystem L2 and hard-bounds it to that many GiB. It retains derived prompt KV on disk, so use encrypted local NVMe where available and make best-effort secure termination available at startup. |
