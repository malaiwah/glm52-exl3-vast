...
SERIAL_TILE_CONFIG = (64, 256, 64, 256)
MIXED_TILE_CONFIG = (128, 128, 32, 512)
...
def _prepared(bits: int, experts: int, seed: int, device: torch.device):
    ...
    return prepare_trellis256_moe_weights(
        ...
        num_experts=experts,
        activation="silu",
        fc1_tile_n=SERIAL_TILE_CONFIG[1],
        fc2_tile_n=SERIAL_TILE_CONFIG[3],
        device=device,
        seed=seed,
        ...
        tile_config=SERIAL_TILE_CONFIG,
    )
...
def _serial_state(m, tier, expert_map, props, device, block_size_m):
    ...
    launch = compile_w4a16_fused_moe(
        ...
        force_tile_config=SERIAL_TILE_CONFIG,
    ...
    return buffers, launch, topk_sum, expert_map, block_size_m


def _run_serial(state, x, weights, ids):
    tier, buffers, launch, topk_sum, expert_map, block_size_m = state
...
def main() -> None:
    ...
    parser.add_argument("--serial-block-m", type=int, default=8)
    parser.add_argument("--mixed-block-m", type=int, default=8)
    parser.add_argument(
        "--variant",
    ...
    for m in args.m:
        ...
        if args.variant in ("both", "serial", "serial-overlap"):
            state0 = (
                tier0,
                *_serial_state(
                    m, tier0, map0, props, device, args.serial_block_m
                ),
            )
            state1 = (
                tier1,
                *_serial_state(
                    m, tier1, map1, props, device, args.serial_block_m
                ),
            )
        ...
        if args.variant in ("both", "mixed"):
            route_slots = max_packed_route_slots(
                m * TOPK, args.mixed_block_m, TOTAL_EXPERTS
            )
            launch = compile_mixed_trellis(
                ...
                top_k=TOPK,
                max_m_blocks=(route_slots + args.mixed_block_m - 1)
                // args.mixed_block_m,
                moe_block_size=args.mixed_block_m,
                sms=int(props.multi_processor_count),
                max_shared_mem=int(props.shared_memory_per_block_optin),
                force_tile_config=MIXED_TILE_CONFIG,
            )
