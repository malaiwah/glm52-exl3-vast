...
| config-state | `config.json`, `known-good.json`, `apply-state.json`, `verify-last.json`, `checkpoint-baseline.json`, and every preserved failure directory. **Not** `terminate-switches.json`: the worker re-checks the kill switch after the erase and immediately before the destroy, so shredding the ratchet first would blind that check to a lock thrown mid-flight. It holds two booleans and dies with the container. |
| derived-kv-cache | all of `<MODEL_ROOT>/.lmcache`, including the bounded filesystem L2. It contains derived prompt KV and may reveal session material even though it is not human-readable text. |
